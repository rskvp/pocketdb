...
func (a *Menu) Query(ctx context.Context, params sch.MenuQueryParam) (*sch.MenuQueryResult, error) {
	params.Pagination = false

	result, err := a.MenuDAL.Query(ctx, params, sch.MenuQueryOptions{
...
func (a *Menu) appendChildren(ctx context.Context, data sch.Menus) (sch.Menus, error) {
	if len(data) == 0 {
		return data, nil
	}
	...
	for _, item := range data {
		childResult, err := a.MenuDAL.Query(ctx, sch.MenuQueryParam{
...
func (a *Menu) Get(ctx context.Context, id string) (*sch.Menu, error) {
	...
	menuResResult, err := a.MenuResourceDAL.Query(ctx, sch.MenuResourceQueryParam{
		MenuID: menu.ID,
	})
	if err != nil {
		return nil, err
...
func (a *Menu) Create(ctx context.Context, formItem *sch.MenuForm) (*sch.Menu, error) {
	menu := &sch.Menu{
		ID:        util.NewXID(),
		CreatedAt: time.Now(),
	}
...
func (a *Menu) Update(ctx context.Context, id string, formItem *sch.MenuForm) error {
	menu, err := a.MenuDAL.Get(ctx, id)
	if err != nil {
		return err
	} else if menu == nil {
