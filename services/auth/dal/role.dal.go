...
func GetRoleDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(sch.Role))
}
...
func (a *Role) Query(ctx context.Context, params sch.RoleQueryParam, opts ...sch.RoleQueryOptions) (*sch.RoleQueryResult, error) {
	var opt sch.RoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
	...
	queryResult := &sch.RoleQueryResult{
...
func (a *Role) Get(ctx context.Context, id string, opts ...sch.RoleQueryOptions) (*sch.Role, error) {
	var opt sch.RoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
...
// Create a new role.
func (a *Role) Create(ctx context.Context, item *sch.Role) error {
	result := GetRoleDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified role in the database.
func (a *Role) Update(ctx context.Context, item *sch.Role) error {
...
func (a *Role) Delete(ctx context.Context, id string) error {
	result := GetRoleDB(ctx, a.DB).Where("id=?", id).Delete(new(sch.Role))
	return errors.WithStack(result.Error)
}
