...
func GetUserRoleDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(sch.UserRole))
}
...
func (a *UserRole) Query(ctx context.Context, params sch.UserRoleQueryParam, opts ...sch.UserRoleQueryOptions) (*sch.UserRoleQueryResult, error) {
	var opt sch.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	db := a.DB.Table(fmt.Sprintf("%s AS a", sch.UserRole{}.TableName()))
	if opt.JoinRole {
		db = db.Joins(fmt.Sprintf("left join %s b on a.role_id=b.id", sch.Role{}.TableName()))
	...
	var list sch.UserRoles
	pageResult, err := util.WrapPageQuery(ctx, db, params.PaginationParam, opt.QueryOptions, &list)
	if err != nil {
		return nil, errors.WithStack(err)
	}

	queryResult := &sch.UserRoleQueryResult{
...
func (a *UserRole) Get(ctx context.Context, id string, opts ...sch.UserRoleQueryOptions) (*sch.UserRole, error) {
	var opt sch.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	item := new(sch.UserRole)
...
func (a *UserRole) Create(ctx context.Context, item *sch.UserRole) error {
	result := GetUserRoleDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified user role in the database.
func (a *UserRole) Update(ctx context.Context, item *sch.UserRole) error {
...
func (a *UserRole) Delete(ctx context.Context, id string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("id=?", id).Delete(new(sch.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByUserID(ctx context.Context, userID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("user_id=?", userID).Delete(new(sch.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByRoleID(ctx context.Context, roleID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("role_id=?", roleID).Delete(new(sch.UserRole))
	return errors.WithStack(result.Error)
}
