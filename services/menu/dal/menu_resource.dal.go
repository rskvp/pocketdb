...
func GetMenuResourceDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(sch.MenuResource))
}
...
func (a *MenuResource) Query(ctx context.Context, params sch.MenuResourceQueryParam, opts ...sch.MenuResourceQueryOptions) (*sch.MenuResourceQueryResult, error) {
	var opt sch.MenuResourceQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
	...
	var list sch.MenuResources
	pageResult, err := util.WrapPageQuery(ctx, db, params.PaginationParam, opt.QueryOptions, &list)
	if err != nil {
		return nil, errors.WithStack(err)
	}

	queryResult := &sch.MenuResourceQueryResult{
...
func (a *MenuResource) Get(ctx context.Context, id string, opts ...sch.MenuResourceQueryOptions) (*sch.MenuResource, error) {
	var opt sch.MenuResourceQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	item := new(sch.MenuResource)
...
func (a *MenuResource) Create(ctx context.Context, item *sch.MenuResource) error {
	result := GetMenuResourceDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified menu resource in the database.
func (a *MenuResource) Update(ctx context.Context, item *sch.MenuResource) error {
...
func (a *MenuResource) Delete(ctx context.Context, id string) error {
	result := GetMenuResourceDB(ctx, a.DB).Where("id=?", id).Delete(new(sch.MenuResource))
	return errors.WithStack(result.Error)
}

// Deletes the menu resource by menu id.
func (a *MenuResource) DeleteByMenuID(ctx context.Context, menuID string) error {
	result := GetMenuResourceDB(ctx, a.DB).Where("menu_id=?", menuID).Delete(new(sch.MenuResource))
	return errors.WithStack(result.Error)
}
